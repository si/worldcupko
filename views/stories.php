...
<?php else: ?>
  <?php 
    foreach($json->events as $event) : 
      ...
      ?>
      <div class="event <?php echo 'venue-' . normalise($event->location) . ' ' . ' date-' . normalise(date('j F Y', strtotime($event->start))) . ' ' . normalise(((strlen($event->group)==1) ? 'Group ' : '') . $event->group); ?>">
        <h2>
