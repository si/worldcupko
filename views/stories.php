...
<?php else: 
var_dump(count($json[0]->Event));
?>
  <?php if(count($json[0]->Event)==0) : ?>
    <p class="info">That's it for another four years. See you in Russia!</p>
  <?php 
    else:
    foreach($json[0]->Event as $event) : 
      ...
      ?>
      <div class="event <?php echo normalise($event->HomeTeam->name) . '-v-' . normalise($event->AwayTeam->name) . ' team-' . normalise($event->home_team->name) . ' team-' . normalise($event->away_team->name) . ' venue-' . normalise($event->location) . ' ' . ' date-' . normalise(date('j F Y', strtotime($event->start))) . ' ' . normalise(((strlen($event->group)==1) ? 'Group ' : '') . $event->group); ?>">
        <h3>
